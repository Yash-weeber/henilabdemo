...
# Sample data based on Figure 5 in the paper
steps = np.arange(0, 300, 5)
optimal_q = 100 * np.ones(len(steps))
dqn = 100 + 50 * np.exp(-steps/150) * np.sin(steps/10) + 30
ddqn = 100 - 20 * np.exp(-steps/100) + 10 * np.random.normal(0, 1, len(steps))
dqnclipped = 100 - 30 * np.exp(-steps/120) + 8 * np.random.normal(0, 1, len(steps))
dqnreg = 100 - 40 * np.exp(-steps/150) + 5 * np.random.normal(0, 1, len(steps))

plt.figure(figsize=(10, 6))
plt.axhline(y=100, color='black', linestyle='-', label='Optimal Q-Value', alpha=0.7)
plt.plot(steps, dqn, label='DQN', color='blue')
plt.plot(steps, ddqn, label='DDQN', color='green')
plt.plot(steps, dqnclipped, label='DQNClipped (Ours)', color='red')
plt.plot(steps, dqnreg, label='DQNReg (Ours)', color='purple')

plt.xlabel('Training Steps')
plt.ylabel('Q-Value Estimate')
plt.title('Q-Value Estimation During Training')
plt.legend()
plt.grid(True, alpha=0.3)
plt.annotate('Overestimation', xy=(150, 140), xytext=(180, 160),
            arrowprops=dict(facecolor='black', shrink=0.05))
plt.annotate('Healthy underestimation', xy=(150, 70), xytext=(180, 50),
            arrowprops=dict(facecolor='black', shrink=0.05))
plt.savefig('q_value_estimation.png', dpi=300)
plt.show()
